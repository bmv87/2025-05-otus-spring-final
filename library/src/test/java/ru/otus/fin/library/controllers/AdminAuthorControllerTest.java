...
import ru.otus.fin.library.controllers.admin.AdminAuthorController;
...
public class AdminAuthorControllerTest {
    ...
    private final String routePath = "/api/v1/admin/authors";
    ...
    @DisplayName("Читатель получит 403 " +
            "при получении списка авторов из контроллера для администратора")
    @Test
    ...
    @DisplayName("Любой неаутентифицированный пользователь получит 401 " +
            "при получении списка авторов из контроллера для администратора")
    @Test
    ...
    @DisplayName("Администратор может получить список авторов " +
    ...
    void shouldReturnListWithOkForAdmin() throws Exception {
        when(authorService.getAdminList(anyString())).thenReturn(new ArrayList<>());
        mvc.perform(get(routePath))
